...
///
/// /version answers without authentication and without a client-version header,
/// which is the entire point of it. A desktop that is too old has to be able to
/// find that out and say so — "this server needs Foxfire 0.13" — before
/// somebody types a password and gets an error they cannot act on. There is no
...
public static class VersionEndpoints
{
    ...
    public static void MapVersionEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/health", (RiotClient riot) =>
            {
                // Degraded rather than unhealthy when Riot has refused the key.
                // Everything already stored still reads; what is lost is new
                // data. A container orchestrator restarting the server over this
                // would achieve nothing — the key is configuration, and the new
                // process would come up holding the same dead one.
