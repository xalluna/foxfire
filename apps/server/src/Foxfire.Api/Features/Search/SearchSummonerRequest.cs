...
///
/// The cost is that a search is thirteen Riot requests out of a budget shared
/// by everybody, which is why it runs at interactive priority and fetches ten
/// games rather than a page. Somebody is watching a spinner, and ten games is
/// enough to see who they are.
/// </summary>
public sealed record SearchSummonerRequest(string GameName, string TagLine)
    : IValidatedRequest<AdHocSummonerResponse>;

internal sealed class SearchSummonerRequestValidator : AbstractValidator<SearchSummonerRequest>
{
    public SearchSummonerRequestValidator() =>
        RuleFor(x => x)
...
internal sealed class SearchSummonerRequestHandler(RiotClient riot)
    : IValidatedRequestHandler<SearchSummonerRequest, AdHocSummonerResponse>
{
    private const int AdHocMatchCount = 10;

    public async Task<Response<AdHocSummonerResponse>> Handle(
        SearchSummonerRequest request,
        CancellationToken cancellationToken)
    {
        ...
        {
            return Response<AdHocSummonerResponse>.Failure(
                new Error(
        ...
        return new AdHocSummonerResponse(
            new AdHocProfile(
                account.Puuid,
            ...
            [
                .. entries
                    .Where(e => RankedQueues.FromRiotName(e.QueueType) is not null)
    ...
    ///
    /// Mirrors what the stored query returns, so an ad-hoc result renders
    /// through exactly the same row as a tracked account's history. The two
    /// fields that cannot apply are the reason this is worth spelling out: LP is
    /// only ever derived for accounts with recorded readings, and search stores
    /// nothing, so an ad-hoc row has no rank and nothing for the editor to act
    /// on.
    /// </summary>
