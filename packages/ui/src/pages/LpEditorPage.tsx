...
import { ladderPosition, queueFilterLabel, queueIdForQueueType } from '@foxfire/core'
import type { EditableMatch, ManualRank, ManualRankEdit, QueueType } from '@foxfire/core'
...
export function LpEditorPage({
  ...
  queueType: QueueType
  matches: EditableMatch[] | undefined
  loading: boolean
  /** The game the editor was opened on, scrolled to and picked out. */
  focusedMatchId: string | null
  /** Stores the entries. Resolves once the fresh list has landed, and rejects with why not. */
  onSave: (edits: ManualRankEdit[]) => Promise<void>
}): JSX.Element {
  ...
  useEffect(() => {
    if (!focusedMatchId || !loaded) return
    rowRefs.current.get(focusedMatchId)?.scrollIntoView({ block: 'center' })
  }, [focusedMatchId, loaded])

  const save = {
    isPending: saving,
    mutate: (edits: ManualRankEdit[]): void => {
      setSaving(true)
      onSave(edits)
        .then(() => {
          setDrafts({})
